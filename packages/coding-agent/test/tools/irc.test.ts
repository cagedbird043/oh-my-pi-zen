...
describe("IRC", () => {
	...
	describe("AgentSession.deliverIrcMessage", () => {
		it("wakes an idle session with a real turn and emits the irc_message event", async () => {
			...
			if (!promptArgs || !Array.isArray(promptArgs[0])) throw new Error("Expected batched IRC prompt");
			const prompted = promptArgs[0][0];
			if (!prompted) throw new Error("Expected incoming IRC message");
