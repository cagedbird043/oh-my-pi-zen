...
- Unicode Snapcompact adds an exact-string side channel for byte-sensitive facts that vision models often misread in dense pixel text. Paths, commands, hashes, architecture tokens, symbols, versions, and URI refs may render as short `[E###]` anchors in the image layer while the exact `E### kind value` mapping is carried as text in the same archive context.
