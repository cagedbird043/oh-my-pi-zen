...
For headless or remote setups backed by a shared auth broker, the CLI exposes `omp auth-broker login <provider>` / `omp auth-broker logout` (and `status`, `list`, `import`, `migrate`). See [Credentials and Auth Cookbook](./credentials.md) for copy-paste commands and [Auth Broker and Auth Gateway](./auth-broker-gateway.md) for the broker model.
